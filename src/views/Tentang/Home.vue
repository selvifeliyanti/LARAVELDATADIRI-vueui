<template>
  ...
  <tbody>
    <tr v-for="(tentang, index) in tentang" :key="index">
      <td>{{ tentang.id}}</td>
      ...
      <td>{{ tentang.email}}</td>
      <td>
        <router-link class="btn btn-success" to="/edittentang">Edit</router-link>
...
export default {
  ...
  setup(){
    let tentang = ref([])
    onMounted(() => {
      axios.get('http://127.0.0.1:8000/api/tentang')
      .then(response => {
        tentang.value = response.data.data
    })
    .catch(error =>{
      console.log(error)
    })
